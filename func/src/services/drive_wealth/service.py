...
class DriveWealthService:
    ...
    async def registry_update_client(
        ...
        if need_create_account:
            ...
            await ValhallaService.register_user_portfolio_us(
                unique_id=unique_id, dw_account=account_id, dw_id=user_dw_id
            )
            await PortfolioRepository.save_unique_id_by_account(
    ...
    async def _create_user_account(cls, user_dw_id: str):
        status, response = await DWTransport.call_registry_account_post(
            user_id=user_dw_id,
    ...
    async def _send_user_document(cls, user_data: dict, user_dw_id: str):
        ...
        document_front = FileRepository.get_file_as_base_64(
            file_type=UserFileType.DOCUMENT_FRONT,
            unique_id=user_unique_id,
            bucket_name=config("AWS_BUCKET_USERS_FILES"),
        )
        document_back = FileRepository.get_file_as_base_64(
        ...
        save_user_document_front = DWTransport.call_save_user_document_file_post(
            user_id=user_dw_id,
            document_type=document_type,
            document=document_front_base_64,
            side=DriveWealthFileSide.FRONT,
        )
        save_user_document_back = DWTransport.call_save_user_document_file_post(
            user_id=user_dw_id,
            document_type=document_type,
